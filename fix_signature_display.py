# -*- coding: utf-8 -*-
"""
Fix the signature display in the downloaded HTML viewer.
The viewer.html inside .epi is created BEFORE signing, so we need to
inject the signed manifest into the extracted HTML.
"""
...
# Complete new demo cell source code with signature injection
NEW_DEMO_SOURCE = '''# @title Install + Record AI Decision { display-mode: "form" }
import sys, time, re
from pathlib import Path
...
if epi_file:
    ...
    if viewer_html:
        # The viewer.html has stale manifest data (created before signing)
        # We need to inject the CURRENT signed manifest
        
        # Create updated embedded data with SIGNED manifest
        updated_data = {
            "manifest": manifest,  # This now has the signature!
            "steps": steps
        }
        ...
        print(f"\\nExtracted: {viewer_html_file.name} (WITH SIGNATURE!)")
    else:
        print("\\nNote: No viewer.html found in .epi archive")
...
for cell in nb['cells']:
    if cell.get('metadata', {}).get('id') == 'demo':
        ...
        cell['source'] = NEW_DEMO_SOURCE.split('\n')
        # Add newlines back (except last line)
        cell['source'] = [line + '\n' for line in cell['source'][:-1]] + [cell['source'][-1]]
        print("[OK] Replaced demo cell with signature-injection fix")
        break
