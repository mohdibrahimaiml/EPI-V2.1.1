# -*- coding: utf-8 -*-
"""
Complete rewrite of the demo cell to fix the dual download issue.
"""
...
# Complete new demo cell source code
    ...
    with zipfile.ZipFile(temp_file, 'r') as z:
        if 'manifest.json' in z.namelist():
            ...
            if sig:
                ...
                display(HTML('<div style="background:#fef2f2;color:#dc2626;padding:20px;margin:10px 0;border-radius:8px;font-weight:bold;">WARNING: File is UNSIGNED! Check auto_sign parameter.</div>'))
...
if epi_file:
    ...
    with zipfile.ZipFile(epi_file, 'r') as z:
        if 'viewer.html' in z.namelist():
            ...
            print(f"\\nExtracted: {viewer_html_file.name} (opens in any browser!)")
        else:
...
for cell in nb['cells']:
    if cell.get('metadata', {}).get('id') == 'demo':
        ...
        print("[OK] Replaced demo cell with fixed version")
        break
...
print("\nFIXES APPLIED:")
print("  1. Correct indentation for all code")
print("  2. Proper dual download logic")
print("  3. Clear extraction of viewer.html")
print("\nDownloads TWO files:")
